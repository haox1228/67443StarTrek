...
print("\(femaleOfficers.count/humanOfficers.count)")



// Q2: How many first officers are human?
var humans = 0
for(key, _value) in firstOfficers{
	if(humanOfficers.contains(key)){
		humans = humans+1
	}
}
print(humans)
...
var officerArr = [Officer]()
for(key,value) in captains{
	officerArr.append(Officer(name: key,ship: value))
}
...
var vulcanFirst = [Officer]()
for(key, value) in firstOfficers{
	if(vulcanOfficers.contains(key)){
		vulcanFirst.append(Officer(name:key, ship:value))
	}
}
...
var vulcanCount = 0
for officer in vulcanFirst {
	if(officer.name.count<5){
		vulcanCount = vulcanCount+1
	}
}
print(vulcanCount)
